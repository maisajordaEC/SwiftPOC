...
//     distribution.
//
// I made the following modifications to this code
// * fixed typecast warnings
// * updated deprecated variables and functions
// * removed UIAlertView on error, this is now handled at the viewcontroller level
// * fixed issue, state = playing is not assigned about 10% of the time
// - Paulo de la Vina
...
- (void)enqueueBuffer
{
	@synchronized(self)
	{
			...
			(state == AS_STOPPED && stopReason == AS_STOPPING_TEMPORARILY))
		{
			...
			//
			if (state == AS_FLUSHING_EOF || buffersUsed ==  (kNumAQBufs/2 - 1))
			{
	...
	ioFlags:(UInt32 *)ioFlags
{
	@synchronized(self)
	{
		...
		else if (inPropertyID == kAudioFileStreamProperty_DataOffset)
		{
			...
			{
				fileLength = dataOffset + (NSInteger)audioDataByteCount;
			}
		}
